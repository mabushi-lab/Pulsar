...
Optional, and off unless a webhook URL is set under **Alerts** in Settings. When set, the device posts a `{"text": "..."}` body - the shape Slack's incoming webhooks expect, and a form most self-hosted relays accept - the moment one of six conditions starts or clears: a probable stock split, a loan symbol matching no position, loan equity going negative, the loan-funded return dropping below what the loan itself costs, a held position that has failed to price for `STALE_ALERT_CYCLES` fetch cycles in a row (`config.h`; 3 by default), or a fund drifted past the same threshold the Portfolio and Allocation screens already draw in orange. The two loan conditions are the Loan screen's own equity and return-vs-cost figures - already colour-coded red or green there - alerted on rather than only shown; both hold their last known state, rather than flipping to "fine", while a price or FX rate needed to compute them is missing. Each condition fires once at the change, not on every refresh cycle, so an unresolved one does not turn into a ping every fifteen seconds. A **Send test alert** button next to the field posts a fixed message immediately, so setting the URL up does not mean waiting for a real warning to find out it works.
...
None of the six latches survive a reboot - an OTA push included. Any condition still true when the device comes back re-fires exactly once, the same as if it had just started; it does not know it already told you.
